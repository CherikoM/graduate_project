<template>
  <div class="audit-styles" ref="auditArtistDOM">

    <h2 class="h2-title">
      风格审核界面
    </h2>

    <div class="audit-styles-main">

      <div class="release-compare">
        <h3>
          <span v-if="!allData.new">对比</span>{{ allData.styleEnName }}({{ allData.new ? "新风格" : allData.style?.id }})
        </h3>
        <div class="compare-container"
          :style="{ 'grid-template-columns': allData.new ? 'repeat(1, 100%)' : 'repeat(2, 50%)' }">
          <div class="now-version">
            <ShowStyleVersion :info="nowVersion" v-if="!allData.new">
              当前版本
            </ShowStyleVersion>
          </div>
          <div class="new-version">
            <ShowStyleVersion :info="newVersion" :change="allData.change">
              新版本
            </ShowStyleVersion>
          </div>
        </div>
...
const submit = async (isPass: boolean, initCb: Function) => {
  const styleData = {
    ...
    enName: newVersion.value.enName,
    otherName: newVersion.value.otherName? JSON.stringify(newVersion.value.otherName): null,
    type: newVersion.value.type *1,
  ...
  const historyData = {
    id: allData.value.id,
    styleId: nowVersion.value.id,
    styleEnName: allData.value.styleEnName,
    auditorId: auditorId,
