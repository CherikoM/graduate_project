...
/**
 * 摘自https://blog.csdn.net/Helenzhn/article/details/124468801
 * 并使用同作者提供的开源敏感词库
 */
@Component
public class SensitiveWordUtil {
    ...
    /**
     * 初始化敏感词库，传入敏感词集合，构建DFA算法模型
     * synchronized是悲观、独占、非公平、可重入锁
    ...
    private static void initSensitiveWordMap(Set<String> sensitiveWordSet) {
        ...
        //迭代sensitiveWordSet（敏感词恐惧而）
        Iterator<String> iterator = sensitiveWordSet.iterator();
        //遍历敏感词集合
        while (iterator.hasNext()) {
            //取得一个敏感词
            key = iterator.next();
            //操作的集合设为sensitiveWordMap
            ...
            for (int i = 0; i < key.length(); i++) {
                ...
                if (wordMap != null) {
                    ...
                    //不存在，则构建一个map，同时将isEnd设置为0，由于他不是最后一个
    ...
    /**
     * 检查文字中是否包含敏感字符
     * 只从开始坐标（beginIndex）处检查一个敏感词，返回敏感词的长度
     * @param txt
     * @param beginIndex
     * @param matchType
    ...
    private static int checkSensitiveWord(String txt, int beginIndex, int matchType) {
        ...
        for (int i = beginIndex; i < txt.length(); i++) {
            ...
            if (nowMap != null) {//存在匹配字，则判断是否为最后一个
                //找到相应key，匹配标识+1
                matchFlag++;
